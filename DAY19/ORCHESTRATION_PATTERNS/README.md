...
> **📊 Diagram rendering** — Architecture diagrams in this file use [Mermaid](https://mermaid.js.org/) syntax. They render automatically on GitHub, GitLab, Notion, and any Markdown viewer with Mermaid support. For local viewing use [VS Code Markdown Preview](https://marketplace.visualstudio.com/items?itemName=bierner.markdown-mermaid) or the [Mermaid Live Editor](https://mermaid.live).
...
1. [Overview](#overview)
2. [Orchestration Patterns — Quick Reference](#orchestration-patterns--quick-reference)
3. [Scenario 1 — Claims Adjudication](#scenario-1--claims-adjudication)
4. [Scenario 2 — Buyer's Research Assistant](#scenario-2--buyers-research-assistant)
5. [Scenario 3 — RFP Response Builder](#scenario-3--rfp-response-builder)
6. [Pattern Comparison Matrix](#pattern-comparison-matrix)
7. [Architecture Decision Guide](#architecture-decision-guide)
...
Use this decision tree to select the right pattern for a new scenario:

```
Is the task structure (agents, connections) known before execution starts?
│
├── YES ──▶ Is execution parallel, sequential, or both?
│             │
│             ├── PARALLEL ──▶ Do all branches need to complete before proceeding?
│             │                  ├── YES ──▶ GraphFlow (parallel fan-out + join)
│             │                  └── NO  ──▶ Selector (route to best single agent)
│             │
│             ├── SEQUENTIAL ──▶ Can any stage loop back to a prior stage?
│             │                    ├── YES ──▶ GraphFlow (sequential + conditional back-edges)
│             │                    └── NO  ──▶ GraphFlow (linear pipeline)
│             │
│             └── BOTH ──▶ GraphFlow (mixed DAG)
│
└── NO  ──▶ Does the coordinator need autonomous planning and tool selection?
              │
              ├── YES ──▶ Magentic (LLM-driven planner)
              │
              └── NO  ──▶ Does the coordinator spawn typed workers by category?
                            ├── YES ──▶ Swarm / Handoff
                            └── NO  ──▶ Round-robin (homogeneous load balancing)
```
...
### LangGraph (GraphFlow Implementation)

GraphFlow patterns map directly to LangGraph's `StateGraph`:

```python
from langgraph.graph import StateGraph, END

# Scenario 1 — parallel fan-out
graph = StateGraph(ClaimState)
graph.add_node("intake",           intake_agent)
graph.add_node("fraud_screening",  fraud_agent)
graph.add_node("policy_coverage",  policy_agent)
graph.add_node("medical_coding",   coding_agent)
graph.add_node("join_barrier",     join_node)
graph.add_node("decision",         decision_agent)

graph.add_edge("intake", "fraud_screening")
graph.add_edge("intake", "policy_coverage")
graph.add_edge("intake", "medical_coding")
graph.add_edge("fraud_screening", "join_barrier")
graph.add_edge("policy_coverage", "join_barrier")
graph.add_edge("medical_coding",  "join_barrier")
graph.add_edge("join_barrier",    "decision")
graph.add_edge("decision",        END)

# Scenario 3 — conditional loop-back
graph.add_conditional_edges(
    "reviewer",
    reviewer_router,          # returns section name or "approved"
    {
        "approved":    "final_signoff",
...
### Swarm / Handoff (Scenario 2)

```python
# Coordinator drives a dynamic loop
async def coordinator(state: ResearchState) -> ResearchState:
    while len(state.confirmed_materials) < TARGET_COUNT:
        results = await web_search_worker(state.query)
        new_materials = extract_materials(results)
        for material in new_materials:
            supplier_data = await supplier_lookup_worker(material)
            pricing_data  = await pricing_worker(material)
            state.confirmed_materials.append({
                "material": material,
                "suppliers": supplier_data,
                "pricing":   pricing_data
            })
    state.report = synthesise(state.confirmed_materials)
    return state
```
