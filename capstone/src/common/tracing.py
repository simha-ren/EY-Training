"""Distributed tracing for the pipeline (the "what happened in this one run" layer).

This complements — it does NOT replace — the Prometheus metrics in
observability.py. Metrics answer "how many / how fast" in aggregate (scraped by
Prometheus/Grafana locally); tracing answers "what happened inside this specific
run" as a trace tree: pipeline -> each agent -> the LLM call, carrying latency,
token usage, RAGAS scores, the quality gate, and guardrail hits.

Provider is auto-selected from the environment, in priority order:
    1. Azure Monitor (Application Insights)  <-- PRODUCTION DEFAULT
       APPLICATIONINSIGHTS_CONNECTION_STRING=...
       On Azure this is injected automatically by the Bicep template
       (infra/deploy/azure/main.bicep provisions App Insights), so production
       tracing needs zero manual configuration.
    2. LangSmith (opt-in, great for LLM-level traces in development)
       LANGCHAIN_TRACING_V2=true  LANGCHAIN_API_KEY=ls-...  [LANGCHAIN_PROJECT=...]
    3. Langfuse (opt-in, open-source / self-hostable)
       LANGFUSE_PUBLIC_KEY=pk-...  LANGFUSE_SECRET_KEY=sk-...  [LANGFUSE_HOST=...]
With none set, tracing is a no-op so the app still runs.
"""
...
def get_tracer():
    """Singleton tracer chosen from the environment
    (Azure Monitor > LangSmith > Langfuse > none)."""
    global _TRACER
