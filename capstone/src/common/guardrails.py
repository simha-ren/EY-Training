...
def _norm_cfg(cfg) -> Dict:
    """Accept a DomainConfig or a plain dict and return a flat policy dict with
    keys: refuse_patterns, refusal_message, disclaimer, clarify."""
    if cfg is None:
        return {}
    if hasattr(cfg, "guardrails"):  # DomainConfig dataclass
        gr = dict(getattr(cfg, "guardrails", {}) or {})
        gr.setdefault("clarify", getattr(cfg, "clarify", {}) or {})
        return gr
    if isinstance(cfg, dict):
        # A nested {"guardrails": {...}, "clarify": {...}} shape or an already-flat one.
        if "guardrails" in cfg and isinstance(cfg["guardrails"], dict):
            merged = dict(cfg["guardrails"])
            merged.setdefault("clarify", cfg.get("clarify", {}) or {})
            return merged
        return cfg
    return {}


def refuse(cfg=None, query: str = "") -> Optional[str]:
...
def detect_gap(cfg=None, query: str = "") -> Optional[str]:
