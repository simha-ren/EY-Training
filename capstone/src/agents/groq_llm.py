"""Groq (and any OpenAI-compatible) LLM integration.

Reconstructed to match the ClaudeLLMClient interface exactly so the two are
drop-in interchangeable (see core/llm_backend.py). Groq exposes an
OpenAI-compatible Chat Completions API, so we use the ``openai`` SDK pointed at
Groq's base URL. Works the same for Ollama / vLLM / any OpenAI-compatible
server by passing a different ``base_url``.

Resilient by design: with no key (or on any API error) it transparently falls
back to a deterministic, grounded extractive mode so chat always answers. The
``online`` flag and ``last_error`` let the UI show which mode produced an answer.
"""
...
class GroqLLMClient:
    """OpenAI-compatible LLM client (Groq by default) with an offline fallback."""
    ...
    def __init__(self, api_key: Optional[str] = None,
        ...
        self.client = None
        self.online = bool(self.api_key) or self.base_url != GROQ_BASE_URL
        if self.online:
            try:
    ...
    def complete(self, system: str, user: str, temperature: float = 0.2,
        ...
        """Get a completion. Returns None (and sets last_error) when offline or
        on any API error, so callers can fall back."""
        if not self.online or self.client is None:
            self.last_error = self.last_error or "No LLM API key configured (offline mode)."
            return None
        try:
    ...
    def chat(self, messages: List[dict], system: str = "", temperature: float = 0.7,
        ...
        try:
            ...
            self.last_error = str(e)
            print(f"Error in chat ({self.provider}): {e}")
            return None
...
"""
        ...
        if response:
            ...
            if clar:
                return self._clarify(clar, mode=self.provider)
            ...
            return {
                "answer": answer_text, "confidence": round(float(confidence), 3),
                "mode": self.provider, "source": source,
                "needs_clarification": False, "used_fallback": False,
        ...
        if fb["confidence"] <= 0.3:
            return self._clarify(
                ...
                mode="offline")
        return {
            "answer": fb["answer"], "confidence": fb["confidence"],
    ...
    def _clarify(message: str, mode: str = "offline") -> dict:
        return {
            "answer": message, "confidence": 0.0, "mode": mode, "source": "clarify",
            "needs_clarification": True, "used_fallback": mode == "offline",
